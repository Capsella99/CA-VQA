...
class DiViDeAddEvaluator(nn.Module):
    ...
    def forward(self, vclips, clip_feat, inference=True, return_pooled_feats=False, reduce_scores=True, pooled=False, **kwargs):
        if inference:
            self.eval()
            with torch.no_grad():
                outputs = self.clip_model(**clip_feat)
                clip_feat = outputs.logits_per_image.view(8, 5).unsqueeze(0).expand(4, -1, -1)
                scores = []
                feats = {}
                for key in vclips:
                    ...
                    if hasattr(self, key.split("_")[0]+"_head"):
                        ...
                        scores += [getattr(self, "vqa_head")(feat, clip_feat)]
            ...
            feats = {}
            outputs = self.clip_model(**clip_feat)
            clip_feat = outputs.logits_per_image.view(-1, 8 ,5)
            for key in vclips:
                feat = getattr(self, key.split("_")[0]+"_backbone")(vclips[key], multi=self.multi, layer=self.layer, **kwargs)
                if hasattr(self, key.split("_")[0]+"_head"):
                    scores += [getattr(self, key.split("_")[0]+"_head")(feat)]
