    ...
    def __getitem__(self, index):
        ...
        for i in range(total_frames):
            ...
            if i % target == 0:
                ...
                frame = torch.tensor(frame).permute(2, 0, 1)  # Convert to tensor and permute to [3, 224, 224]
        ...
        # if youtube UGC
        clip_frames = []
        video_folder = os.path.join(filename.split(".mp4")[0] + "_frames")
        frame_files = glob.glob(os.path.join(video_folder, 'frame_*.jpg'))
        ...
        clip_frames = torch.stack(clip_frames, dim=1)  # Stack frames to get shape [3, 8, 224, 224]
