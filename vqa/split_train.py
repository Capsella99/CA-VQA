...
def inference_set(inf_loader, model, device, best_, save_model=False, suffix='s', save_name="divide"):
    ...
    if s + p > best_s + best_p and save_model:
        ...
        torch.save(
            ...
            f"pretrained_weights/{save_name}_{suffix}_dev_v0.2.pth",
        )
...
def main():
    ...
    for split in range(1,num_splits):
        ...
        if opt["pretrained_clip"]:
            model.clip_model =  CLIPModel.from_pretrained("../results/checkpoint-1265").to(device)
        else:
