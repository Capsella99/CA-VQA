...
prompt = ["bad aesthetic picture","poor aesthetic picture","fair aesthetic picture","good aesthetic picture","excellent aesthetic picture"]

"""
prompt = [
    "bad lighting picture",
    "poor lighting picture",
    "fair lighting picture",
    "good lighting picture",
    "excellent lighting picture",
    "bad color picture",
    "poor color picture",
    "fair color picture",
    "good color picture",
    "excellent color picture",
    "bad composition picture",
    "poor composition picture",
    "fair composition picture",
    "good composition picture",
    "excellent composition picture",
    "bad content picture",
    "poor content picture",
    "fair content picture",
    "good content picture",
    "excellent content picture"
]
"""

#prompt = ["bad aesthetic picture","good aesthetic picture"]
...
def finetune_epoch(ft_loader, model, model_ema, optimizer, scheduler, device, epoch=-1, 
    ...
    for i, data in enumerate(tqdm(ft_loader, desc=f"Training in epoch {epoch}")):
        ...
        frames = data["clip_frames"]
        frames_flat = frames.view(-1, 3, 224, 224)
        clip_inputs = clip_processor(text=prompt, images=frames_flat, return_tensors="pt", padding=True)
        ...
        if need_feat:
            ...
            scores = model(video, clip_feat=clip_inputs, inference=False,
                                  reduce_scores=False) 
    ...
    for i, param_group in enumerate(optimizer.param_groups):
        ...
        print(f"Epoch [{epoch}], Param Group {i+1}, LR: {current_lr:.6f}")
...
def inference_set(inf_loader, model, device, best_, save_model=False, suffix='s', save_name="divide"):
    ...
    for i, data in enumerate(tqdm(inf_loader, desc="Validating")):
        ...
        for key in sample_types:
            ...
            #.unsqueeze(0)
        with torch.no_grad():
            result["pr_labels"] = model(video,clip_feat=clip_inputs).cpu().numpy()
            if len(list(video_up.keys())) > 0:
                result["pr_labels_up"] = model(video_up,clip_feat=clip_inputs).cpu().numpy()
                
        result["gt_label"] = data["gt_label"].item()
        result["file_name"] = file_name[0]
        del video, video_up
    ...
    for k in range(10):
        print(results[k]["file_name"])
        print("label",gt_labels[k])
        print("pred",pr_labels[k])
        print("---")
    ...
    # Function to determine color based on resolution
    def get_color(file_name):
        if "720P" in file_name:
            return 'blue'
        elif "1080P" in file_name:
            return 'green'
        elif "2160P" in file_name:
            return 'red'
        elif "480P" in file_name:
            return 'black'
        else:
            return 'gray'

    colors = [get_color(name) for name in file_names]

    # Create scatter plot
    plt.figure(figsize=(10, 6))
    min_val = min(min(gt_labels), min(pr_labels))
    max_val = max(max(gt_labels), max(pr_labels))
    plt.plot([min_val, max_val], [min_val, max_val], color='black', linewidth=1, label='Ideal Correlation')

    plt.scatter(gt_labels, pr_labels, c=colors, alpha=0.8)
    plt.xlabel('Ground Truth')
    plt.ylabel('Predicted')
    plt.title('Scatter Plot of Predicted vs Ground Truth')
    plt.grid(True)


    # Create legend
    legend_labels = {'720P': 'blue', '1080P': 'green', '2160P': 'red', "480P": "black", "360P": "gray"}
    handles = [plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=color, markersize=10) for color in legend_labels.values()]
    plt.legend(handles, list(legend_labels.keys()))

    plt.savefig("YTUGC.png")
    exit(0)
    #######################    
...
def main():
    ...
    for split in range(1,num_splits):
        ...
        if "load_path_aux" in opt:
            ...
            model.clip_model =  CLIPModel.from_pretrained("openai/clip-vit-base-patch16").to(device)
        ...
        for key, value in dict(model.named_children()).items():
            if "backbone" in key:
                ...
                param_groups += [{"params": value.parameters(), "lr": opt["optimizer"]["lr"] * opt["optimizer"]["backbone_lr_mult"] * opt["clip_lr_mult"]}]
            else:
        ...
        for key, value in dict(model.named_children()).items():
            ...
            if "clip" in key:
                for param in value.vision_model.parameters():
                    param.requires_grad = False
                for param in value.text_model.parameters():
                    param.requires_grad = False
